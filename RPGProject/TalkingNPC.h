...
public:
    ...
    DialogueNode(string NPCtalk, vector<string> choices, vector<int> next)
    ...
    vector<string> getPlayerChoices() const { return playerChoices; }
...
public:
  ...
  void printDialogue(int currentIndex) {
      ...
      //check to see if the user has arrived at a boring ending node and end the game if so
      if (currentIndex == 10) {
        cout << "You spend a peaceful night studying. The world might need saving, but your grades probably need you more.";
        exit(0);
      }
...
public:
  hostileGuyNPC() : TalkingNPC("Jerk", 10) {
    ...
    dialogueTree[0] = new DialogueNode(
      "Get outta my way!",
      { "Calm down", "Man, you were in my way", "Who are you ?" },
      { 1,2,3 }
    );

    dialogueTree[1] = new DialogueNode(
      "Why should I?",
      { "It's good for your mental health", "It's not a big deal" },
      { 4,5 }
    );

    dialogueTree[2] = new DialogueNode(
      "You have a lot of nerve trying to tell me what to do.",
      { "I'm on a very important mission. Goodbye", "I was just making a simple observation" },
      { 7,8 }
    );

    dialogueTree[3] = new DialogueNode(
      "Someone very important.",
      { "I don't believe that", "Sorry, Mr.VIP.I didn't mean to interrupt your day." },
      { 6,9 }
    );
...
public:
  englishProfessorNPC() :TalkingNPC("English Professor", 3) {
    ...
    dialogueTree[1] = new DialogueNode(
      ...
      {"Press Enter to exit the conversation"},
      {}
    );
